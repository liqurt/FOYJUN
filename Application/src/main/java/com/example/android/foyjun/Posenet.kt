...
class Posenet(
        val context: Context,
        val filename: String = "posenet_model.tflite", // 있어봤자 이거..?
    ...
    fun estimateSinglePose(bitmap: Bitmap): Person {
        val estimationStartTimeNanos = SystemClock.elapsedRealtimeNanos() //별 중요한거 아닌듯.
        val inputArray = arrayOf(initInputArray(bitmap)) //뭐 하는건진 잘 모르겠음
        Log.i(
                "posenet",
                String.format(
        ...
        getInterpreter().runForMultipleInputsOutputs(inputArray, outputMap) //여기가 추론하는 곳인걸로 판단됨
        ...
        // return person에다가 중단점을 걸고 디버그창에서 변수 확인
        return person
    }
}